...
s1 = Scenario.create({title: '6 to 4 base', scenario_type: Scenario::SCENARIO_TYPES[0]})
...
RountingRule.create({
  ...
  var interfaceOut = nodeTo.interface
...
RountingRule.create({
  ...
  if (message.getDestProtocol() === 'ipv6' && message.getSourceProtocol() === 'ipv4') {
    ...
    var sourceIPv6 = "3FFE:A1::5EFE:" + message.source;
    var destIPv4 = nodeTo.interfaceIn.ip_address;
